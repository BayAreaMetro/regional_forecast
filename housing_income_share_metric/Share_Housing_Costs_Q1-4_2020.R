# Share_Housing_Costs_Q1-4_2020.R
# Create matrix for 2050 share of housing costs, starting with 2019 assumptions
# Note that this script uses 2019 PUMS values (because a viable PUMS 2020 doesn't exist) and 
# 2020 UrbanSim ones. As such, there's a mix of 2019 and 2020-named variables
# SI
...
# Share of total households by unit type in 2019 (taken from 2010 values in below document)
...
# Bring in 2019 PUMS data on share of income spent by renters/owners for different quartiles, from script:
# https://github.com/BayAreaMetro/census-tools-for-planning/blob/master/Analysis/ACS%20PUMS%202019/ACS_2019_PUMS_Share_Income_on_Housing_Costs_by_Quartile.R
# Bring in other relevant files: Urbansim run, scenario specific parameters

pums2019            <- read.csv(pums_2019_location,header=TRUE)

county_2020         <- read.csv(County_2020_Loc,header=TRUE,stringsAsFactors = FALSE) %>% 
...
## Fill in hh_proportion_matrix_2020 table
...
rent_share <- pums2019 %>%                         # Use PUMS data to distribute 2020 run quartiles by owner/renter
...
temp20p <- county_2020 %>%  # Populate 2020 housing totals by income and tenure, join with shell above
...
temp20p <- temp20p %>% mutate(  # Now fill in cells
  tt=case_when(
    hu_type=="dr"        ~   temp20p[5,"tt"]*drshare,    # Calculate total deed restricted units in 2020
    hu_type=="su"        ~   temp20p[5,"tt"]*sushare,    # Calculate total subsidized units in 2020
    hu_type=="pc"        ~   temp20p[5,"tt"]*pcshare,    # Calculate total price control units in 2020
    hu_type=="ma"        ~   temp20p[5,"tt"]*mashare,    # Calculate total market units in 2020
    hu_type=="total"     ~   tt
  ),
  tr=case_when(
...
hh_proportion_matrix_2020 <- full_2020 %>% 
...
## Fill in hh_income_matrix_2020 table

temp20i <- pums2019 %>%  # Populate 2019 share income spent, join with shell from above
...
# Now calculate 2020 market share of income spent on housing (requires flexing your 8th grade algebra skills) 
"
...
hh_income_matrix_2020 <- temp20i %>% mutate(
  q1r=case_when(
    hu_type=="ma"        ~ ((full_2020[5,"q1r"]*.[5,"q1r"])-            # Now use weighted average to determine market share of income spent
                              ((full_2020[1,"q1r"]*.[1,"q1r"])+
                                 (full_2020[2,"q1r"]*.[2,"q1r"])+
                                 (full_2020[3,"q1r"]*.[3,"q1r"])))/
                                 ((full_2020[3,"q1r"]*pc_fac1r)+full_2020[4,"q1r"]),
    TRUE                 ~ q1r),
  
  q2r=case_when(
    hu_type=="ma"        ~ ((full_2020[5,"q2r"]*.[5,"q2r"])-            # Now use weighted average to determine market share of income spent
                              ((full_2020[1,"q2r"]*.[1,"q2r"])+
                                 (full_2020[2,"q2r"]*.[2,"q2r"])+
                                 (full_2020[3,"q2r"]*.[3,"q2r"])))/
                                 ((full_2020[3,"q2r"]*pc_fac2r)+full_2020[4,"q2r"]),
    TRUE                 ~ q2r),
  
  q3r=case_when(
    hu_type=="ma"        ~ ((full_2020[5,"q3r"]*.[5,"q3r"])-            # Now use weighted average to determine market share of income spent
                              ((full_2020[1,"q3r"]*.[1,"q3r"])+
                                 (full_2020[2,"q3r"]*.[2,"q3r"])+
                                 (full_2020[3,"q3r"]*.[3,"q3r"])))/
                                 ((full_2020[3,"q3r"]*pc_fac3r)+full_2020[4,"q3r"]),
    TRUE                 ~ q3r),

  q4r=case_when(
    hu_type=="ma"        ~ ((full_2020[5,"q4r"]*.[5,"q4r"])-            # Now use weighted average to determine market share of income spent
                              ((full_2020[1,"q4r"]*.[1,"q4r"])+
                                 (full_2020[2,"q4r"]*.[2,"q4r"])+
                                 (full_2020[3,"q4r"]*.[3,"q4r"])))/
                                 ((full_2020[3,"q4r"]*pc_fac4r)+full_2020[4,"q4r"]),
    TRUE                 ~ q4r),
  
  q1o=case_when(
    hu_type=="ma"        ~ ((full_2020[5,"q1o"]*.[5,"q1o"])-            # Now use weighted average to determine market share of income spent
                              ((full_2020[1,"q1o"]*.[1,"q1o"])+
                                 (full_2020[2,"q1o"]*.[2,"q1o"])+
                                 (full_2020[3,"q1o"]*.[3,"q1o"])))/
                                 ((full_2020[3,"q1o"]*pc_fac1o)+full_2020[4,"q1o"]),
    TRUE                 ~ q1o),
  
  q2o=case_when(
    hu_type=="ma"        ~ ((full_2020[5,"q2o"]*.[5,"q2o"])-            # Now use weighted average to determine market share of income spent
                              ((full_2020[1,"q2o"]*.[1,"q2o"])+
                                 (full_2020[2,"q2o"]*.[2,"q2o"])+
                                 (full_2020[3,"q2o"]*.[3,"q2o"])))/
                                 ((full_2020[3,"q2o"]*pc_fac2o)+full_2020[4,"q2o"]),
    TRUE                 ~ q2o),
  
  q3o=case_when(
    hu_type=="ma"        ~ ((full_2020[5,"q3o"]*.[5,"q3o"])-            # Now use weighted average to determine market share of income spent
                              ((full_2020[1,"q3o"]*.[1,"q3o"])+
                                 (full_2020[2,"q3o"]*.[2,"q3o"])+
                                 (full_2020[3,"q3o"]*.[3,"q3o"])))/
                                 ((full_2020[3,"q3o"]*pc_fac3o)+full_2020[4,"q3o"]),
    TRUE                 ~ q3o),
  
  q4o=case_when(
    hu_type=="ma"        ~ ((full_2020[5,"q4o"]*.[5,"q4o"])-            # Now use weighted average to determine market share of income spent
                              ((full_2020[1,"q4o"]*.[1,"q4o"])+
                                 (full_2020[2,"q4o"]*.[2,"q4o"])+
                                 (full_2020[3,"q4o"]*.[3,"q4o"])))/
                                 ((full_2020[3,"q4o"]*pc_fac4o)+full_2020[4,"q4o"]),
    TRUE                 ~ q4o),
...
temp50p <- county_2050 %>%  # Populate 2050 housing totals by income and tenure, join with shell above
  cbind(.,rent_share) %>% mutate(       # Join renter shares from PUMS 2019
    hu_type="total",
...
temp50p <- temp50p %>% mutate(  # Now fill in cells
  tr=case_when(
    ...
    hu_type=="su"        ~   round((full_2020[2,"tr"]/full_2020[5,"tt"])*
                             county_2050[1,"TOTHH"]),  # Apply proportion from 2020 to 2050 su totals
    hu_type=="pc"        ~   rpc_2050,
    TRUE                 ~   tr),                     
  to=case_when(
    hu_type=="dr"        ~   odr_2050,                 # Apply 2050 values from Scenario-Specific Values
    hu_type=="su"        ~   round((full_2020[2,"to"]/full_2020[5,"tt"])*
                                     county_2050[1,"TOTHH"]),
    hu_type=="pc"        ~   opc_2050,
    TRUE                 ~   to), 
  )
...
# Start w/ 2020, make changes indicated by Forecasting Income Share Spent on Housing in 2050 memo

hh_income_matrix_2050     <- hh_income_matrix_2020 %>% mutate(
  q1r=case_when(
    hu_type=="dr"        ~   q1r,                
    hu_type=="su"        ~   q1r,                  
    hu_type=="pc"        ~   q1r*price_2050_to_2020,
    hu_type=="ma"        ~   q1r*price_2050_to_2020),
  q2r=case_when(
    hu_type=="dr"        ~   q2r,                
    hu_type=="su"        ~   q2r,                  
    hu_type=="pc"        ~   q2r*price_2050_to_2020,
    hu_type=="ma"        ~   q2r*price_2050_to_2020),
  q3r=case_when(
    hu_type=="dr"        ~   q3r,                
    hu_type=="su"        ~   q3r,                  
    hu_type=="pc"        ~   q3r*price_2050_to_2020,
    hu_type=="ma"        ~   q3r*price_2050_to_2020),
  q4r=case_when(
    hu_type=="dr"        ~   q4r,                
    hu_type=="su"        ~   q4r,                  
    hu_type=="pc"        ~   q4r*price_2050_to_2020,
    hu_type=="ma"        ~   q4r*price_2050_to_2020),
  q1o=case_when(
    hu_type=="dr"        ~   q1o,
    hu_type=="su"        ~   q1o,
    hu_type=="pc"        ~   q1o,
    hu_type=="ma"        ~   q1o*price_2050_to_2020),
  q2o=case_when(
    hu_type=="dr"        ~   q2o,
    hu_type=="su"        ~   q2o,
    hu_type=="pc"        ~   q2o,
    hu_type=="ma"        ~   q2o*price_2050_to_2020),
  q3o=case_when(
    hu_type=="dr"        ~   q3o,
    hu_type=="su"        ~   q3o,
    hu_type=="pc"        ~   q3o,
    hu_type=="ma"        ~   q3o*price_2050_to_2020),
  q4o=case_when(
    hu_type=="dr"        ~   q4o,
    hu_type=="su"        ~   q4o,
    hu_type=="pc"        ~   q4o,
    hu_type=="ma"        ~   q4o*price_2050_to_2020)
) %>% 
