...
csv_file_path <-
  file.path(
    getwd(),
    'Documents/GitHub/regional_forecast/contextual/output/bayareahalopop_components_long_2020_2022.csv'
  )
pdf_file_path <-
  file.path(
    getwd(),
    'Documents/GitHub/regional_forecast/contextual/plots/bayareahalopop_components_long_2020_2022.pdf'
  )
...
popest_tot <-
  popest %>% filter(year > 2012) %>% group_by(region_2, year) %>%
